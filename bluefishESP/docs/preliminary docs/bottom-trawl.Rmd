...
# 
# saveRDS(dat, file = here::here("data-raw/nmfs_survey_expanded_9c.RDS"))

# dat <- readRDS(here::here("data-raw/nmfs_survey_expanded_9c.RDS"))
...
all_tows <- all_nmfs %>%
  ...
  dplyr::filter(#WEEK %in% c(10:12, 37:43)# march, sept, oct
    SEASON == "FALL"
    ) %>% 
  dplyr::select(SEASON, #MONTH, WEEK, 
                EST_TOWDATE, LAT, LON, SURFTEMP, SURFSALIN, YEAR) %>%
